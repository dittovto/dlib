...
// ----------------------------------------------------------------------------------------
    ...
    inline std::vector<image_window::overlay_line> render_half_face_detections (
        const std::vector<full_object_detection>& dets,
        const rgb_pixel color = rgb_pixel(0,255,0)
    )
    {
        ...
        for (unsigned long i = 0; i < dets.size(); ++i)
        {
            ...
            else
            {
                ...
                // Line on top of nose
                for (unsigned long i = 9; i < 13; ++i)
                    lines.push_back(image_window::overlay_line(d.part(i), d.part(i+1), color));

                // left eyebrow
                for (unsigned long i = 14; i < 17; ++i)
                    lines.push_back(image_window::overlay_line(d.part(i), d.part(i+1), color));
                // Right eyebrow
                for (unsigned long i = 18; i < 20; ++i)
                    lines.push_back(image_window::overlay_line(d.part(i), d.part(i+1), color));
                // Bottom part of the nose
                for (unsigned long i = 21; i < 26; ++i)
                    lines.push_back(image_window::overlay_line(d.part(i), d.part(i+1), color));
                // Line from the nose to the bottom part above
                lines.push_back(image_window::overlay_line(d.part(26), d.part(21), color));

                // Left eye
                for (unsigned long i = 27; i < 30; ++i)
                    lines.push_back(image_window::overlay_line(d.part(i), d.part(i+1), color));
                lines.push_back(image_window::overlay_line(d.part(30), d.part(31), color));

                // Right eye
                for (unsigned long i = 31; i < 34; ++i)
                    lines.push_back(image_window::overlay_line(d.part(i), d.part(i+1), color));
                lines.push_back(image_window::overlay_line(d.part(33), d.part(27), color));

                // Lips outer part
                for (unsigned long i = 34; i < 36; ++i)
                    lines.push_back(image_window::overlay_line(d.part(i), d.part(i+1), color));
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        std::vector<full_object_detection> dets;
        dets.push_back(det);
        return render_half_face_detections(dets, color);
    }
}
